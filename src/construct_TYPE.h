...
#define _construct_TYPE_h
    ...
    struct BkuPayload
    {
        ...
        BkuPayload(TYPE_ TYPE, double wert):TYPE(TYPE), wert(wert){
            size ++;
        }
        ~BkuPayload(){
            size --;
    ...
    int BkuPayload::size = 0;


    int BkuPayloadlength(BkuPayload *mypayload){
        int length = 0;
    ...
    uint8_t *BkuPayloadconverter(BkuPayload *mypayload){
        // uint8_t *buildPayload = nullptr;
        // delete[] buildPayload;
        int uebertrag = BkuPayloadlength(mypayload);
    ...
    void BkuPrintPayload(BkuPayload *mypayload)
    {	
        //uint8_t MSG[] = {uint8_t(payloadconverter(mypayload))};
        uint8_t MSG[] = {*BkuPayloadconverter(mypayload)};
        for (size_t i = 0; i < BkuPayloadlength(mypayload); i++)
        {
